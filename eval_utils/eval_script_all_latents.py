...
def parse_args():
    ...
    parser.add_argument('--latents_log', default='/work/dlclarge1/ramans-cmbrl/contextual_mbrl/logs/', type=str, help='Path to the latents file') #_pgm_ctx carl_classic_cartpole_single_0_enc_img_dec_img_normalized /work/dlclarge1/ramans-cmbrl/contextual_mbrl/logs/carl_classic_cartpole_double_box_enc_img_dec_img_pgm_ctx_adv_normalized
    parser.add_argument('--output_file', type=str, help='Path to the output file')
    return parser.parse_args()


class LatentsDataset(Dataset):
    def __init__(self, latent_dir, state_keys = ['imagined',], target_key = 'length', state_split ='all', transform=None):
        ''''
            ...
            for file in files:
                if file.endswith('ctx2latent_v1.pkl'):
                    ...
                    for context_id, per_context_dict in enumerate(data):
                        ...
                        context = per_context_dict['context'][self.target_key]
...
def create_torch_dataset(latent_dir, target = 'length'):
    latents_ds = LatentsDataset(latent_dir, target_key=target)
    return latents_ds
...
def visualize_pca(latents_ds, save_dir=None):
    ...
    if save_dir:
        ...
        fig.write_html(os.path.join(save_dir, 'pca_plots', os.path.basename(latents_ds.latent_dir) + latents_ds.target_key + '.html'))
...
def linear_probes(latents_ds, save_dir=None):
    ...
    df['target'] = latents_ds.target_key
    #print("df: ", df.head(10))
    #evaluate the model
    ...
    df = pd.DataFrame({'score': score, 'experiment': os.path.basename(latents_ds.latent_dir), 'target': latents_ds.target_key}, index=[0])
    if save_dir:
...
def latent_analysis(data_dir, target, save_dir):

    latents_ds = create_torch_dataset(data_dir, target)
    print("loaded latents from file: ", data_dir)
...
def main():
    args = parse_args()
    save_dir = os.path.join(os.path.dirname(args.latents_log), 'latents_analysis')
        #make directory for analysis if it does not exist
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    #loop over all subdirectories and load the latents
    print("Loading latents from file: ", args.latents_log)
    for p_subdir in os.listdir(args.latents_log):
        if os.path.isdir(os.path.join(args.latents_log, p_subdir)):
            print("p_subdir: ", p_subdir)
            current_subdir = os.path.join(args.latents_log, p_subdir)
            try: 
                if 'single' in current_subdir:
                    match = re.search(r'single_(\d+)', current_subdir)
                    print("match: ", match.group(1))
                    if int(match.group(1)):
                        target = 'actuator_strength' if 'walker' in current_subdir else 'length'
                    else:
                        target = 'gravity'
                elif 'double' in current_subdir:
                    target = ['actuator_strength' if 'walker' in current_subdir else 'length',
                               'gravity']
                ...
                if isinstance(target, list):
                    for t in target:
                        latent_analysis(current_subdir, t, save_dir)
                elif isinstance(target, str):
                    latent_analysis(current_subdir, target, save_dir)

                #get the target from the subdirectory name
